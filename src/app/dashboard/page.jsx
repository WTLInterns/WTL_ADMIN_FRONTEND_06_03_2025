...
const Dashboard = () => {
  ...
  const fetchBookings = async () => {
    setLoading(true);
    try {
      const response = await fetch('https://worldtriplink.com/details'); // Adjust API URL if needed
      if (!response.ok) {
  ...
  const deleteBooking = async (bookingId) => {
    try {
      const response = await fetch(`https://worldtriplink.com/details/${bookingId}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
