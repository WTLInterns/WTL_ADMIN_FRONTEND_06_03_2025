...
export default function AllB2B() {
  ...
  const fetchB2BList = async () => {
    try {
      const res = await fetch("https://worldtriplink.com/b2b/all");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch("https://worldtriplink.com/b2b/add", {
        method: "POST",
        body: formDataToSend,
      });
      if (!response.ok) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`https://worldtriplink.com/b2b/delete/${id}`, {
        method: "DELETE",
      });
